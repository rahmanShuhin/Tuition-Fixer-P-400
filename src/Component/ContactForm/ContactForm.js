...
const ContactForm = ({
  ...
  const handleForm = (e) => {
    ...
    if (contacts) {
      ...
      fetch(
        `https://calm-shore-19939.herokuapp.com/update/tutor/contact/${id}`,
        {
          method: "PATCH",
  ...
  const handleReview = (e) => {
    ...
    fetch(`https://calm-shore-19939.herokuapp.com/update/feedback/${id}`, {
      method: "PATCH",
      body: JSON.stringify({
      ...
      .then((json) => {
        const newArr = teacher.filter((x) => {
          if (x._id === id) {
            x.tuition.star =
              (x.tuition.star * (dup.length - 1) + rate) / dup.length;
  ...
  const handleUpdate = (e) => {
    ...
    fetch(
      `https://calm-shore-19939.herokuapp.com/update/myProfile/${user.email}`,
      {
        method: "PATCH",
  ...
  const handleContactPost = (data) => {
    console.log(data);
    fetch("https://calm-shore-19939.herokuapp.com/post/contact", {
      method: "POST",
    ...
    <div className="form">
      ...
      {contact === "update" && (
        <form onSubmit={handleUpdate}>
          ...
          <Select
            label="Status"
            defaultValue={profile?.tuition?.available}
            required
          >
